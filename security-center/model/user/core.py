...
class Core(BaseModel):
    ...
    # 认证时间
    auth_time = DateTimeField(formats=DATETIME_FMT, null=True)
    # 注销时间
    close_time = DateTimeField(formats=DATETIME_FMT, null=True)
    ...
    # 用户创建当前别名的时间
    _cnickname_time = DateTimeField(formats=DATETIME_FMT, default=datetime.now)

    # 用户密码
    _password = CharField()
    # 用户创建当前密码的时间
    _cpassword_time = DateTimeField(formats=DATETIME_FMT, default=datetime.now)

    # 用户邮箱
    _email = CharField()
    # 用户当前邮箱写入时间
    _cemail_time = DateTimeField(formats=DATETIME_FMT, default=datetime.now)

    # 用户手机号
    _phone = CharField(null=True)
    # 用户当前写入手机号时间
    _cphone_time = DateTimeField(formats=DATETIME_FMT, null=True)

    # 用户个人信息
    info = BinaryJSONField(default=info_default)
    ...
    # 用户信息修改时间
    changed_history = BinaryJSONField(default=changed_history_default)
    # 用户登录历史记录
